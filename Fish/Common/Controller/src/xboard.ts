...
import { BoardPosition, Board } from "../types/board";
import { InvalidNumberOfPlayersError, InvalidBoardConstraintsError, InvalidPositionError } from "../types/errors";
 ...
 */
const getNumReachableFromBoard = (arrayBoard: number[][], position: number[]): number => {
    const board: Board | InvalidBoardConstraintsError | InvalidPositionError = createNumberedBoard(arrayBoard);
    ...
    if (!isError(board)) {
        ...
        if (!isError(game)) {
            const startPosition: BoardPosition = { row: position[0], col: position[1]};
